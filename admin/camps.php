<?php
	...
	if(isset($_POST["submit"]) && $_POST["submit"] == "camps"){
		unset($_POST["submit"]);
		$say = $db-> query("SELECT count(*) FROM term WHERE name = '".$_POST["name"]."'");
		if($say[0]["count(*)"] < 1){
        	$db->insert("term",$_POST);
			$text = "<div style=\"background:green;color:#fff;width:100%;border:1px solid #ccc;border-radius:5px;padding:5px;margin-bottom:20px;\">Düşərgə adı əlavə olundu</div>";
		}else{
			$text = "<div style=\"background:red;color:#fff;width:100%;border:1px solid #ccc;border-radius:5px;padding:5px;margin-bottom:20px;\">Düşərgə adı bazada var</div>";
		}
	}
